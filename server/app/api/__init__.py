...
def create_app() -> FastAPI:
            ...
            with session_scope() as session:
                ...
                if request_id:
                    ...
                    data_service.update_command_request_status(
                        ...
                        status=status,
                        response_payload=payload_text or payload_for_log,
                        error=None if success else str(payload_json.get('error') or 'failed'),
                        device_id=device_id,
                    )
        except Exception:  # pragma: no cover - 外部服务
    ...
    mqtt_manager.register_handler(
        MQTT_TOPICS['cableway_command_response'],
        command_response_handler,
    )
    mqtt_manager.register_handler(
    ...
    mqtt_manager.register_handler(
        ...
        lambda context: handle_device_hello(context, mqtt_manager),
    )
    ...
    def cableway_status_handler(context: MQTTMessageContext) -> None:
        handle_cableway_status_payload(context, mqtt_manager=mqtt_manager)

    mqtt_manager.register_handler(MQTT_TOPICS['cableway_status'], cableway_status_handler)
