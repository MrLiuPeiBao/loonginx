...
## 1. 项目定位
本系统用于工业安全监测与现场控制：
- **下位机（loongarch64 GNU/Linux，小端，Python 3.9）**：轮询 RS485 传感器、采集 BMS/RFID、与 PLC 通信，并通过 MQTT 上报。
...
- `client/communication/cableway_plc.py`：PLC（Modbus TCP）状态轮询与命令执行。
...
sequenceDiagram
  ...
  C->>B: 发布 sensors/data 或 cableway/status
  B->>S: 投递消息
  S->>S: 解析/阈值判断/告警生成
  S->>D: 入库
  ...
  U->>S: 下发命令（/api/commands 或 /api/cableway/command）
  S-->>B: MQTT 下发命令
  B-->>C: 下位机收到并执行
  C-->>B: MQTT 回执
  B-->>S: 回执入库/更新命令状态
...
| PLC Modbus TCP | 502 | 下位机到 PLC | `PLC_PORT` |
...
- `cableway/status`：PLC 状态
- `cableway/command/request` / `response`：PLC 命令与回执
...
- 命令：`/api/commands`、`/api/cableway/command`
