# 文件讲解：`server/app/services/audio_worker.py`

## 1. 文件定位
- **角色**：业务/基础模块
- **所在层级**：`server/app/services`
- **是否建议新人优先阅读**：是
- **模块文档摘要**：Standalone audio worker process for isolation from API.
...
- **函数设计**：
  - `main()`
    ...
    - 依赖函数：AudioMonitorService, MQTTManager, get_settings, load_env, mqtt_manager.connect, mqtt_manager.disconnect, service.start, service.stop
