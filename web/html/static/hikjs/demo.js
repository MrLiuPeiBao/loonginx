// 初始化插件。真实设备配置由 config/local/camera-config.js 在本机提供。
const DEVICE_CONFIG = Object.assign({
    ip: "",
    port: "80",
    username: "",
    password: "",
...
// 自动登录函数
function autoLogin() {
    if (!DEVICE_CONFIG.autoLogin) {
        return;
    }

    console.log("开始自动登录设备...");
    updateDeviceStatus("正在连接...");

    var szDeviceIdentify = DEVICE_CONFIG.ip + "_" + DEVICE_CONFIG.port;
    ...
    var iRet = WebVideoCtrl.I_Login(
        ...
        DEVICE_CONFIG.password,
        {
            success: function (xmlDoc) {
                showOPInfo(szDeviceIdentify + " 自动登录成功！");
                updateDeviceStatus("已连接", true);
            ...
            error: function (status, xmlDoc) {
                var errorMsg = szDeviceIdentify + " 自动登录失败！";
                showOPInfo(errorMsg, status, xmlDoc);
...
// 更新设备状态显示
function updateDeviceStatus(status, isConnected = false) {
    var statusElement = $("#deviceStatus");
    statusElement.text(status);
    if (isConnected) {
        statusElement.removeClass().addClass("connected");
    } else {
        statusElement.removeClass().addClass("disconnected");
    }
}
